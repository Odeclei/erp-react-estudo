...
from companies.utils.exceptions import (NotFoundEmployee, NotFoundGroup,
                                        NotFundTasks, NotFundTasksStatus)
...
class Base(APIView):
    ...
    def get_status(self, status_id):
        ...
        if not status:
            raise NotFundTasksStatus
        return status

    def get_task(self, task_id, enterprise_id):
        task = Task.objects.filter(
            id=task_id, enterprise_id=enterprise_id).first()
